...
File.makeDirectory(dir+"results"+resultname);
File.makeDirectory(dir+"result"+ resultname+ "Images");
...
for (ii=0; ii<fileNames.length; ii++){
	if(startsWith(fileNames[ii],"MAX")){
		...
		run("Analyze Particles...", "size="+minCellSize+"-Infinity show=Masks include slice");
		//run("Grays");
		run("Watershed");
		...
		Stack.setChannel(2);
		run("Gaussian Blur...", "radius=1 slice");
	...
	nrSpots=0;
		...
		saveAs("Tiff", dir+"result" + resultname +"Images\\"+name);
		...
		saveAs("Results", dir+"results" + resultname +"\\"+name+".csv");
		run("Close All");
...
selectWindow("Log");
saveAs("Text",dir+"results" + resultname +"\\Log_"+macroName+".txt");
