...
File.makeDirectory(dir+"results_EdU");
File.makeDirectory(dir+"resultImages_EdU");
...
for (ii=0; ii<fileNames.length; ii++){
	if(startsWith(fileNames[ii],"MAX")){
		...
		saveAs("Tiff", dir+"resultImages_EdU\\"+name);
		...
		saveAs("Results", dir+"results_EdU\\"+name+".csv");
		run("Close All");
...
selectWindow("Log");
saveAs("Text",dir+"results_EdU\\Log_"+macroName+".txt");
