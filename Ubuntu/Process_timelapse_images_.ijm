...
for (j=0; j < n_ch.length; j++) {
	...
	for (i = 0; i < n_pos.length; i++) {
		...
		run("Image Sequence...", "open=["+dir+"Position"+IJ.pad(n_pos_string, 3)+"--t00--Z00--C"+IJ.pad(n_ch_string, 2)+".tif] file=(Position"+IJ.pad(n_pos_string, 3)+".*C"+IJ.pad(n_ch_string, 2)+") sort");
