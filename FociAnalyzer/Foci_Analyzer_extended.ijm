 ...
 * 
 * Maarten Paul, April 2024
 * - added center of mass of the nuclei to the results table, so that the location of the nuclei in the images can be retrieved in subsequent analysis steps
*/
...
//TO DO: FIND A BETTER WAY FOR THIS:
	...
	if(nrNuclei > 0) {
		...
		mass_center_x_ = Table.getColumn("MASS_CENTER_X", "Results");
		mass_center_y_ = Table.getColumn("MASS_CENTER_Y", "Results");
		...
		Table.setColumn("Mass center X (pixel)", mass_center_x_, resultTable);
		Table.setColumn("Mass center Y (pixel)", mass_center_y_, resultTable);
