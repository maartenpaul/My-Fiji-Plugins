...
for (i=0; i<filepths.length; i++){
	if(indexOf(filepths[i],"STORM")>0&&endsWith(filepths[i],".lsm")){
		...
		filename = filename[0];
		run("Bio-Formats", "open=["+ dir+File.separator+filename+".lsm] color_mode=Default view=Hyperstack stack_order=XYCZT use_virtual_stack");
		
		//2x threshold for BRCA2 images, 3x threshold for RAD51 images
		if(endsWith(filename,"BRCA2")){
		...
		run("Export results", "floatprecision=5 filepath=["+ dir+File.separator+ filename+"_driftcorr_merged.csv] fileformat=[CSV (comma separated)] sigma=true intensity=true offset=true saveprotocol=true chi2=false x=true y=true bkgstd=true chi2=false id=false uncertainty_xy=true frame=true");
		close();
		close();
	}
}
