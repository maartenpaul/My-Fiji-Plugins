...
File.close(f);
if(nResults>0) run("Clear Results");
print("Import TS data");
run("Import results", "detectmeasurementprotocol=true filepath=["+new_file+"] fileformat=[CSV (comma separated)] livepreview=false rawimagestack= startingframe=1 append=false");
run("Visualization", "imleft=0.0 imtop=0.0 imwidth="+ width + " imheight=" + height +" renderer=[Normalized Gaussian] dxforce=false magnification=20.0 colorize=false dx=20.0 threed=false dzforce=false");
saveAs("Tiff", new_img);
